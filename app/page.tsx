    ...
    return (
      <div className="flex min-h-[calc(100vh-4rem)] items-center justify-center bg-[#0a0a0a]">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-neutral-500">Memuat data dashboard...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-neutral-100 py-8 px-4 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="grid gap-5 grid-cols-2 lg:grid-cols-4 mb-10">
          <div className="rounded-xl border border-neutral-900 bg-neutral-950/20 p-4 sm:p-5 flex flex-col justify-between min-h-[110px]">
            <span className="text-[10px] sm:text-xs font-semibold text-neutral-500 uppercase tracking-wider">Pendapatan Hari Ini</span>
            <div className="flex items-baseline justify-between mt-2 flex-wrap gap-1">
              <h3 className="text-lg sm:text-xl md:text-2xl font-bold text-neutral-200">{formatRupiah(revenueToday)}</h3>
              <TrendingUp className="h-4 w-4 text-emerald-500 shrink-0" />
            </div>
          </div>

          <div className="rounded-xl border border-neutral-900 bg-neutral-950/20 p-4 sm:p-5 flex flex-col justify-between min-h-[110px]">
            <span className="text-[10px] sm:text-xs font-semibold text-neutral-500 uppercase tracking-wider">Pendapatan Bulan Ini</span>
            <div className="flex items-baseline justify-between mt-2 flex-wrap gap-1">
              <h3 className="text-lg sm:text-xl md:text-2xl font-bold text-neutral-200">{formatRupiah(revenueMonth)}</h3>
              <TrendingUp className="h-4 w-4 text-purple-500 shrink-0" />
            </div>
          </div>

          <div className="rounded-xl border border-neutral-900 bg-neutral-950/20 p-4 sm:p-5 flex flex-col justify-between min-h-[110px]">
            <span className="text-[10px] sm:text-xs font-semibold text-neutral-500 uppercase tracking-wider">Total Pendapatan</span>
            <div className="flex items-baseline justify-between mt-2 flex-wrap gap-1">
              <h3 className="text-lg sm:text-xl md:text-2xl font-bold text-neutral-200">{formatRupiah(revenueAllTime)}</h3>
              <TrendingUp className="h-4 w-4 text-indigo-500 shrink-0" />
            </div>
          </div>

          <div className="rounded-xl border border-neutral-900 bg-neutral-950/20 p-4 sm:p-5 flex flex-col justify-between min-h-[110px]">
            <span className="text-[10px] sm:text-xs font-semibold text-neutral-500 uppercase tracking-wider">Customer Aktif</span>
            <div className="flex items-baseline justify-between mt-2 flex-wrap gap-1">
              <h3 className="text-lg sm:text-xl md:text-2xl font-bold text-neutral-200">{activeCustomersCount} Slot</h3>
        ...
        <div className="grid gap-6 md:grid-cols-2">
          ...
          <div className="rounded-xl border border-neutral-900 bg-neutral-950/40 p-5 flex flex-col">
            <h2 className="text-sm font-semibold text-neutral-300 mb-4 flex items-center justify-between">
              <span className="flex items-center gap-2">
                <ShieldAlert className="h-4 w-4 text-amber-500" />
                Customer Akan Habis (≤3 hari)
              </span>
              <span className="text-[10px] text-neutral-500 font-bold bg-neutral-900 px-2 py-0.5 rounded border border-neutral-800">
            ...
            {expiringCustomers.length === 0 ? (
              <div className="flex-1 flex flex-col items-center justify-center py-12 text-center">
                <div className="h-10 w-10 rounded-xl bg-neutral-900 flex items-center justify-center mb-3 text-neutral-600">
                  <ShieldCheck className="h-5 w-5 text-emerald-500" />
                </div>
                <h4 className="font-semibold text-neutral-300 text-xs">Semua Aman</h4>
              ...
              <div className="flex-1 space-y-3 max-h-[350px] overflow-y-auto pr-1">
                {expiringCustomers.map(sub => {
                  ...
                  return (
                    <div key={sub.id} className="flex items-center justify-between p-3 rounded-lg bg-neutral-950/40 border border-neutral-900/60 text-xs">
                      <div className="overflow-hidden pr-2">
                        <strong className="block text-neutral-200 truncate font-semibold">{sub.customer_email}</strong>
                      ...
                      <div className="text-right shrink-0">
                        ...
                        <Link 
                          href="/customers" 
                          className="block text-[10px] text-purple-400 hover:text-purple-300 font-semibold mt-1.5 transition-colors"
                        >
                          Perpanjang &rarr;
                        </Link>
                      </div>
          ...
          <div className="rounded-xl border border-neutral-900 bg-neutral-950/40 p-5 flex flex-col">
            ...
            {alertHosts.length === 0 ? (
              <div className="flex-1 flex flex-col items-center justify-center py-12 text-center">
                ...
                <h4 className="font-semibold text-neutral-300 text-xs">Stok & Akun Aman</h4>
                <p className="text-[10px] text-neutral-500 mt-1 max-w-xs">
              ...
              <div className="flex-1 space-y-3 max-h-[350px] overflow-y-auto pr-1">
                {alertHosts.map(host => {
                  ...
                  return (
                    <div key={host.id} className="p-3 rounded-lg bg-neutral-950/40 border border-neutral-900/60 text-xs space-y-2">
                      <div className="flex justify-between items-start gap-2">
                        <strong className="text-neutral-200 truncate font-semibold block">{host.account_email}</strong>
                        <span className="text-[10px] uppercase font-bold tracking-wider px-1.5 py-0.5 rounded bg-neutral-900 text-neutral-400 border border-neutral-800 shrink-0">
                          {host.billing_type}
                        </span>
                      </div>
